...
export default class Graph {
  ...
  preProcess() {
    ...
    var highlightColor = Color("darkgray");
    var highlightTextColor = highlightColor.isDark() ? "lightyellow" : "black";
    ...
    _.each(this.nodes, (node) => {
      ...
      node.highlightTextColor = highlightTextColor;

      node.uniqueColor = uniqueColor();
      this.colorNodeMapping[node.uniqueColor] = node;

      node.graphText = node.shortMethod;
    });
    ...
    _.each(this.nodes, (node) => {
      ...
      //   method = `${method}...`;
      // }
      var textWidth = this.textWidths[method] = this.textWidths[method] || this.canvasContext.measureText(method).width; 
      if (textWidth > ScaleCache.x(node.width) - ScaleCache.y(0.2) ) {
        node.graphText = "";
      }
  ...
  get fontSize() {
    this._fontSize = this._fontSize || 0.6 * ScaleCache.y(1);
